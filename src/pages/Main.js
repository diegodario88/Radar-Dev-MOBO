...
function Main({ navigation }) {
    ...
    async function loadDevs() {
        ...
        const { latitude, longitude } = currentPos
        const reponse = await Api.get('/api/v1/search', {
            params: {
                latitude,
                longitude,
                techs
    ...
    return (
        <>
            <MapView onRegionChangeComplete={handleRegionChanged} initialRegion={currentPos} style={styles.map} >
                {devs.map(dev => (
                    <Marker
                        key={dev._id}
                        coordinate={{
            ...
            <View style={styles.searchForm}>
                <TextInput
                    ...
                    placeholder='Find the best Dev by Techs...'
